...
CAR_TYPES = { 'Cargo' => CargoCar, 'Passenger' => PassengerCar }
  ...
  def train_load
    ...
    puts "Для выбора необходимого вагона"
  ...
  def train_cars_list(train)
    num = 0
    train.iterate_cars {|c| puts "#{num += 1}: #{c.type} Свободно: #{c.free} Занято: #{c.filled}"}
  rescue RuntimeError => e
    puts "Ошибка: #{e.message}"
    train_menu
  end
  ...
  def station_trains_list
    station = station_choice
    station.iterate_trains {|t| puts "#{t.number} - #{t.type}, Кол-во вагонов: #{t.cars.size}"}
  rescue RuntimeError => e
    puts "Ошибка: #{e.message}"
    info_menu
  end
